<template>
  <div class="diagnosis-chat">
    <el-card shadow="hover">
      ...
      <div class="chat-container">
        ...
        <div class="chat-input-area">
          ...
          <div class="chat-input">
            <el-button class="img-btn" @click="triggerImageUpload" :disabled="loading" title="上传图片">
              📷
            </el-button>
            <input ref="fileInputRef" type="file" accept="image/*" style="display:none" @change="onImageSelect" />
            <el-input
              v-model="inputText"
              placeholder="描述症状或上传舌苔/化验单图片..."
      ...
      <el-alert
        v-if="!apiConfigured"
        title="请先在系统设置中配置 DeepSeek API Key"
        type="warning"
        :closable="false"
    ...
    <el-dialog v-model="showAnalyze" title="🔬 四诊辨证分析" width="700px" destroy-on-close>
      <el-form :model="analyzeForm" label-width="80px">
        ...
        <el-form-item label="问诊">
          <el-input v-model="analyzeForm.inquiry" type="textarea" :rows="3"
            placeholder="寒热、汗出、饮食、睡眠、二便等" />
        </el-form-item>
        <el-row :gutter="16">
          <el-col :span="8">
...
async function onAnalyzeImageSelect(event: Event) {
  ...
  try {
    ...
    if (target === 'lab') {
      analyzeImages.value.lab.push(res.data_url)
    } else {
      analyzeImages.value[target] = res.data_url
    }
  } catch {
    ElMessage.error('图片上传失败')
  }
...
async function sendMessage() {
  ...
  try {
    const payload: any = { message: text, conversation_id: conversationId.value }
    if (images.length) payload.images = images
    const res = await api.post('/diagnosis/chat', payload) as any
...
async function submitAnalyze() {
  ...
  const form = analyzeForm.value
  const imgs = analyzeImages.value
  ...
  try {
    ...
    messages.value.push({
      role: 'assistant',
      content: res.syndrome_analysis,
      warnings: res.safety_warnings || [],
    })
  } catch {
    messages.value.push({ role: 'assistant', content: '抱歉，分析出错，请稍后重试。' })
  }
  loading.value = false
